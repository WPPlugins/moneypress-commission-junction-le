...
function csl_mpcj_activate() {
    ...
    // Check Registration
    //
    if(!$MP_cj_plugin->no_license) {    
        if (!$MP_cj_plugin->license->check_license_key()) {
            $MP_cj_plugin->notifications->add_notice(
                2,
                __('Your license ',MP_CJ_PREFIX). get_option(MP_CJ_PREFIX . '-license_key') . __(' could not be validated.',MP_CJ_PREFIX)
            );        
        }
    }
...
function csl_mpcj_admin_stylesheet() {
    if ( file_exists(MP_CJ_COREDIR.'css/admin.css')) {
        wp_register_style('csl_mpcj_admin_css', MP_CJ_COREURL .'css/admin.css'); 
        wp_enqueue_style ('csl_mpcj_admin_css');
    }  
}
